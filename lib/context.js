...
class ContextSession {
  ...
  /**
   * internal logic of `ctx.session` 获取session
  ...
  /**
   * internal logic of `ctx.session=` 设置session
  ...
  /**
   * init session from cookie 从cookie中初始化session
   * @api private
   */
  initFromCookie() {
  ...
  /**
   * verify session(expired or ) 校验session是否过期、缺失或无效
  ...
  /**
   * create a new session and attach to ctx.sess 创建新session对象
   *
  ...
  /**
   * Commit the session changes or removal. 提交session修改和移除
   *
   * @api public
   */
  async commit() {
